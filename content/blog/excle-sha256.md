+++
title = "在Excel中使用SHA256加密，谷歌广告上传目标客户匹配数据"
date = "2022-06-05T10:41:11+08:00"
description = "谷歌广告推广中，如果上传目标客户匹配数据，可以帮助系统更好地完成机器学习，达到优化获客成本的目的。在上传目标客户匹配数据之前，为了确保数据的机密性和安全性，需要将实际的电子邮件地址和电话号码等信息转换为哈希代码，也就是哈希算法 SHA256。"

tags = ["google-ads",]
+++
谷歌广告推广中，如果上传目标客户匹配数据，可以帮助系统更好地完成机器学习，达到优化获客成本的目的。在上传目标客户匹配数据之前，为了确保数据的机密性和安全性，需要将实际的电子邮件地址和电话号码等信息转换为哈希代码，也就是哈希算法 SHA256。
<!--more-->

## 什么是SHA256哈希算法
SHA256全称为Secure Hash Algorithm 256，代表“安全哈希算法 256 位”。SHA256是一种去中心化算法，用于将任意长度的文本转换为固定长度的 256 位（32 字节）文本。

加密哈希算法将产生不可逆转并且唯一的哈希值，所以谷歌无法将你上传的信息解密。

## 在Excel中使用SHA256加密

可以上传目标客户匹配数据的情形很多，包括线上及线下数据。例如，在搜索网络、“购物”标签页、Gmail、YouTube 和展示广告网络上覆盖和重新吸引客户。这些客户可能包括电子邮件地址、电话号码等。

对Excel中的数据批量SHA256加密可以提高我们的工作效率，本文就教大家如何在Excle中批量SHA256加密。

### 第1步：制作Excle的SHA256加密模版

[下载Excle的SHA256加密模版](/files/FunSHA256.xlsm)

上面是一份现成的Excle的SHA256加密模版，如果你不准备手动制作，直接下载使用即可。**下载好Excle的SHA256加密模版后，可以直接进入下面的第2步。**

ok，那我们现在开始制作Excle的SHA256加密模版。

1. 启用开发工具功能：依次点击，文件->选项->自定义功能区->主选项卡->勾选开发者工具
2. 打开“Visual Basic”编辑器：点击“开发工具”页面的“Visual Basic”或者通过顶部菜单栏的“工具”->宏->Visual Basic 编辑器
3. 插入“类模块”：在打开界面的“工程”空白处点击右键，选择插入类模块，并命名为 `CSHA256`，粘贴下面提供的代码
4. 插入“模块”：空白处点击右键，选择插入“模块”，并命名为 `SHA256`，粘贴下面提供的代码
5. 保存文件为 `FunSHA256.xlsm`

![0605-Excle的SHA256加密模版](/images/2022/0605-Excle的SHA256加密模版.png)
...
### 第2步：使用Excle批量SHA256加密
1. 打开SHA256加密模版，点击确认启用宏，将需要加密的数据复制粘贴到第一列
2. 选择明文列一行数据（如：A3），点击对应加密列（如：B3），调用函数：输入=funSHA256（A3）
3. 在加密完一行数据后，鼠标可直接放在该加密数据右下角，显示黑色十字标示后双击即可完成批量加密
4. 点击另存为CSV格式与UTF-8编码的文件，再次打开该文件，删除第一列（即：明文列）数据，仅保留加密列数据。

![Excle批量SHA256加密数据](/images/2022/0605-Excle批量SHA256加密数据.png)